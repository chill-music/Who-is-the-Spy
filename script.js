...
const SHOP_ITEMS = {
    frames: [
        ...
        { id: 'frame_img', name_en: "Image Frame", name_ar: "إطار صورة", cost: 100, type: 'frames', preview: 'https://i.ibb.co/qMPp6srK/Untitled-2.png' }, 
    ],
    titles: [
            ...
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4" onClick={onClose}> 
                <div className="glass-panel rounded-2xl p-5 w-full max-w-sm animate-pop" onClick={e => e.stopPropagation()}> 
                    <div className="profile-header"> 
                        <div className="relative inline-block mb-2 rounded-full" style={!isFrameUrl && frameItem ? { padding: '3px', background: frameItem.preview } : {}}>
                            ...
                            {isFrameUrl && (
                                <img src={frameItem.preview} className="absolute top-1/2 left-1/2 w-[118%] h-[118%] -translate-x-1/2 -translate-y-1/2 pointer-events-none z-20 rounded-full" alt=""/>
                            )}
                        </div>
            ...
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-2 overflow-y-auto" onClick={onClose}> 
                <div className="glass-panel rounded-2xl p-4 w-full max-w-md animate-pop my-4 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}> 
                    ...
                    <div className="flex items-center gap-3 mb-3 flex-shrink-0">
                        <div className="relative rounded-full" style={!isEqFrameUrl && equippedFrameItem ? { padding: '3px', background: equippedFrameItem.preview } : {}}>
                            ...
                            {isEqFrameUrl && (
                                <img src={equippedFrameItem.preview} className="absolute top-1/2 left-1/2 w-[118%] h-[118%] -translate-x-1/2 -translate-y-1/2 pointer-events-none z-20 rounded-full" alt=""/>
                            )}
                        </div>
                                            ...
                                            {inventory.frames.map(id => {
                                                ...
                                                return (
                                                    <button 
                                                        ...
                                                        className={`w-12 h-12 rounded-full overflow-hidden relative transition ${isEq ? 'ring-2 ring-cyan-400 scale-110' : 'opacity-70 hover:opacity-100'}`}
                                                    >
                                                        {isUrl ? (
...
function App() {
    ...
    const handleJoinFromInvite = async (rId) => { setShowChat(false); setInputCode(rId); setLoading(true); let uid = user?.uid; if (!uid) { const anon = await auth.signInAnonymously(); uid = anon.user.uid; setUser(anon.user); } const ref = roomsCollection.doc(rId); const snap = await ref.get(); if (snap.exists) { const data = snap.data(); const exists = data.players.find(p => p.uid === uid); if (!exists) { await ref.update({ players: [...data.players, { uid: uid, name: nickname, status: 'active', photo: getDefaultPhoto(userData, nickname), role: null, equipped: userData?.equipped || {} }] }); } setRoomId(rId); setActiveView('lobby'); } setLoading(false); };
