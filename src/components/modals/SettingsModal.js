        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content animate-pop" onClick={e => e.stopPropagation()} style={{ maxWidth: '360px' }}>
                ...
                <div className="modal-body">
                    ...
                    {user && !isGuestPropForSettings && myRole && (
                        <div style={{marginBottom:'10px'}}>
                            <button onClick={() => { onClose(); if(onOpenAdminPanel) onOpenAdminPanel(); }}
                                ...
                                <div style={{fontSize:'24px'}}>{myRoleCfg?.icon||'🛡️'}</div>
                                <div style={{flex:1,textAlign:lang==='ar'?'right':'left'}}>
                                    ...
                                    <div style={{fontSize:'10px',color:'#9ca3af',marginTop:'1px'}}>{lang==='ar'?`دخول كـ ${myRoleCfg?.label_ar}`:`Access as ${myRoleCfg?.label_en}`}</div>
                                </div>
                                <div style={{fontSize:'10px',padding:'3px 10px',borderRadius:'6px',fontWeight:700,background:`${myRoleCfg?.color||'#ffd700'}22`,border:`1px solid ${myRoleCfg?.color||'#ffd700'}44`,color:myRoleCfg?.color||'#ffd700'}}>{lang==='ar'?myRoleCfg?.label_ar:myRoleCfg?.label_en}</div>
                            </button>
                        </div>
                    )}
