    ...
    return (/*#__PURE__*/
      ...
      messages.map((msg, i) => {
        ...
        return (/*#__PURE__*/
          ...
          React.createElement("div", { onClick: () => openMiniProfilePub(msg.senderId, { name: msg.senderName, photo: msg.senderPhoto }),
          ...
          React.createElement("input", { autoFocus: true, value: editText, onChange: (e) => setEditText(e.target.value), onKeyDown: (e) => {if (e.key === 'Enter') handleEditMsg(msg.id);if (e.key === 'Escape') {setEditingMsgId(null);setEditText('');}},
            style: { flex: 1, padding: '7px 10px', background: 'rgba(0,242,255,0.08)', border: '1px solid rgba(0,242,255,0.3)', borderRadius: '10px', color: 'white', fontSize: '12px', outline: 'none' } }), /*#__PURE__*/
          ...
          React.createElement("button", { onClick: () => {setEditingMsgId(msg.id);setEditText(msg.text || '');setMenuMsgId(null);}, style: { width: '100%', padding: '7px 10px', borderRadius: '7px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '11px', color: '#00f2ff', fontWeight: 700, textAlign: 'left', display: 'flex', alignItems: 'center', gap: '6px' },
            onMouseEnter: (e) => e.currentTarget.style.background = 'rgba(0,242,255,0.1)',
            onMouseLeave: (e) => e.currentTarget.style.background = 'none' }, "\u270F\uFE0F ", lang === 'ar' ? 'تعديل' : 'Edit'),

          isMe && /*#__PURE__*/
          React.createElement("button", { onClick: () => handleDeleteMsg(msg.id), style: { width: '100%', padding: '7px 10px', borderRadius: '7px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '11px', color: '#f87171', fontWeight: 700, textAlign: 'left', display: 'flex', alignItems: 'center', gap: '6px' },
            onMouseEnter: (e) => e.currentTarget.style.background = 'rgba(239,68,68,0.1)',
            onMouseLeave: (e) => e.currentTarget.style.background = 'none' }, "\uD83D\uDDD1\uFE0F ", lang === 'ar' ? 'حذف' : 'Delete'), /*#__PURE__*/

          React.createElement("button", { onClick: () => handleReport(msg), style: { width: '100%', padding: '7px 10px', borderRadius: '7px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '11px', color: '#9ca3af', fontWeight: 700, textAlign: 'left', display: 'flex', alignItems: 'center', gap: '6px' },
            onMouseEnter: (e) => e.currentTarget.style.background = 'rgba(255,255,255,0.07)',
            onMouseLeave: (e) => e.currentTarget.style.background = 'none' }, "\uD83D\uDEA8 ", lang === 'ar' ? 'إبلاغ' : 'Report')
          )
      ...
      React.createElement("div", { style: { position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 80, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end' },
        onClick: () => setShowRPModal(false) }, /*#__PURE__*/
      React.createElement("div", { style: { width: '100%', background: 'linear-gradient(160deg,#0e0e22,#13122a)', borderRadius: '20px 20px 0 0', border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden', maxHeight: '60%', boxSizing: 'border-box' },
      ...
      React.createElement("button", { key: rp.id, onClick: () => sendPublicRP(rp), disabled: sendingRP || (currentUser?.currency || 0) < rp.amount,
      ...
      React.createElement("input", { ref: inputRef, value: msgText, onChange: (e) => setMsgText(e.target.value), onKeyDown: (e) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), sendMsg()),
        ...
        placeholder: lang === 'ar' ? 'اكتب رسالة للعموم...' : 'Write a public message...' }), /*#__PURE__*/
