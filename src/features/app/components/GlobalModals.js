(function () {
  ...
  window.GlobalModals = function (props) {
    var {
      ...
      nickname, setNickname, setupGameId, setSetupGameId, setupMode, setSetupMode,
    ...
    var [showFriendSelector, setShowFriendSelector] = useState(false);
    var [onFriendSelected, setOnFriendSelected] = useState(null);
    ...
    // --- [INVITE SYSTEM BRIDGE] ---
    window.openPrivateChatWithFriendSelector = function (callback) {
      if (typeof setShowFriendSelector !== 'function') return;
      setOnFriendSelected(() => callback);
    ...
    return (/*#__PURE__*/
      React.createElement("div", {
        ...
        className: "global-modals-container",
        style: {
          position: 'fixed',
          top: 0,
          left: 0,
          ...
          height: '100%',
          zIndex: 999999,
          // ✅ FIX: Container never blocks clicks — each modal overlay handles its own pointerEvents
          pointerEvents: 'none',
          backgroundColor: 'transparent'
        }
      },
        /* Gift Animation Overlay */
        window.GiftAnimationOverlay && /*#__PURE__*/
        React.createElement(window.GiftAnimationOverlay, {
          queue: giftOverlayQueue,
          setQueue: setGiftOverlayQueue,
          lang: lang
        }),
        ...
        React.createElement(window.ProfileV11, {
          ...
          onOpenChat: (target) => {
            ...
            if (target === 'self') { if (setShowSelfChat) setShowSelfChat(true); } else { openPrivateChat && openPrivateChat(target); }
          }
        }
        ),
        ...
        // --- [SPY REJOIN MODAL] ---
        showRejoinSpy && React.createElement("div", {
          className: "modal-overlay animate-fadeIn",
          style: { zIndex: 3000 },
          onClick: () => {
            localStorage.removeItem('last_spy_room_id');
            if (setShowRejoinSpy) setShowRejoinSpy(false);
            if (setRejoinSpyDeclined) setRejoinSpyDeclined(true);
          }
        }, React.createElement("div", {
          className: "modal-content animate-pop",
          style: { maxWidth: '340px', textAlign: 'center', padding: '30px 20px' },
          ...
          React.createElement("h2", { style: { fontSize: '18px', fontWeight: 900, marginBottom: '10px' } },
            lang === 'ar' ? 'لديك لعبة قائمة في "من هو الجاسوس"!' : 'Active Spy Game Found!'),
          React.createElement("p", { style: { fontSize: '13px', color: 'rgba(255,255,255,0.6)', marginBottom: '25px', lineHeight: '1.5' } },
            lang === 'ar' ? 'هل تريد العودة ومواصلة اللعب؟' : 'Would you like to return and continue playing?'),
          React.createElement("div", { style: { display: 'flex', gap: '10px', flexDirection: 'column' } },
            React.createElement("button", {
              className: "btn-neon w-full py-3",
              onClick: () => {
                // Clear any lingering "Mission detected" overlays
                const existingOverlay = document.getElementById('spy-detection-overlay');
                if (existingOverlay) existingOverlay.remove();

                // Explicitly set game and room state
                if (setActiveGameId) setActiveGameId('spy');
                if (setRoomId) setRoomId(rejoinSpyRoomId);
                
                // Activate the new Spy Rebuild UI
                if (setShowSpyRebuild) setShowSpyRebuild(true);
                if (setShowRejoinSpy) setShowRejoinSpy(false);
              }
            }, lang === 'ar' ? 'نعم، عودة' : 'Yes, Rejoin'),
            React.createElement("button", {
        ...
        // --- [FRIEND SELECTOR FOR INVITES] ---
        window.FriendSelectorModal && React.createElement(window.FriendSelectorModal, {
          show: showFriendSelector,
          onClose: () => setShowFriendSelector(false),
          lang: lang,
          friendsData: friendsData,
          onSelect: (friend) => {
            if (onFriendSelected) onFriendSelected(friend);
