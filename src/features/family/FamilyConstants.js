...
// ══════════════════════════════════════════════
// ✅ FAMILY TASKS CONFIG
// Each task MUST have a `points` field — used to add to daily activity bar when claimed.
// `daily: true`  → resets and re-claimable every day (progress tied to today's date)
// `daily: false` → weekly task (progress resets on weekly cycle or manually)
//
// Tracking fields on family doc (taskProgress map):
//   key = `{taskId}_{uid}`
//   value = { current: number, claimed: boolean, lastCheckIn: string (for daily) }
//
// `action`: optional string hint for the UI to show a "Go" button linking to the action
//   'gifts'    → opens gift sending UI
//   'games'    → navigates to game lobby
//   'donate'   → scrolls to donate panel
//   'like'     → navigates to clan members tab
// ══════════════════════════════════════════════
...
// Thresholds: 20 / 60 / 100 daily points
// Each milestone has a `rewards` array for claimDailyMilestoneChest().
// Also carries `intel` shorthand used by legacy code.
// ══════════════════════════════════════════════
