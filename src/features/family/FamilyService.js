...
var openAssignedChest = async ({ family, currentUID, currentUserData, inventoryIdx, lang, onNotification }) => {
    ...
    try {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            var totalClaimsAfter = Object.values(freshChest.claimedBy).reduce((acc, c) => acc + c, 0);
            var maxTotalClaims = (freshChest.assignedTo || []).length * (freshChest.maxClaimsPerMember || 1);
            if (totalClaimsAfter >= maxTotalClaims) {
                freshInv.splice(inventoryIdx, 1);
            } else {
            ...
            // 🛡️ SECURITY: Use centralized service for currency to ensure audit logs
            if (rewardsToGive.currency > 0) {
                if (window.SecurityService) {
                    await window.SecurityService.applyCurrencyTransaction(currentUID, rewardsToGive.currency, `Family Chest: ${freshChest.name_en || 'Treasury Chest'}`, {
                        chestId: freshChest.chestId || freshChest.id || 'unknown',
                        familyId: family.id
                    });
                } else {
                    await usersCollection.doc(currentUID).update({ currency: firebase.firestore.FieldValue.increment(rewardsToGive.currency) });
                }
            }
            ...
            (rewardsToGive.items || []).forEach(r => {
                var normalizedType = (r.type === 'titles') ? 'title' : (r.type === 'badges') ? 'badge' : r.type;
                var itemId = r.frameId || r.titleId || r.badgeId || r.ringId || r.giftId || r.id; // Support generic 'id'
                var defItem = null;
                
                if (window.SHOP_ITEMS && itemId) {
                    // Optimized Multi-Category Lookup
                    var categories = ['frames', 'titles', 'badges', 'rings', 'profileEffects', 'limitedFrames', 'gifts', 'gifts_vip', 'gifts_family', 'gifts_special'];
                    for (var cat of categories) {
                        var list = window.SHOP_ITEMS[cat] || [];
                        var found = list.find(x => x.id === itemId);
                        if (found) {
                            defItem = found;
                            break;
                        }
                ...
                var dur = (r.duration !== undefined) ? r.duration : (r.durationDays !== undefined) ? r.durationDays : (defItem?.durationDays);
                r._computedDuration = dur; // save for receipt
                var expiresAt = dur ? Date.now() + dur * 86400000 : null;
                var expKey = `inventory.expiry.${itemId}`;
                
                // --- Dynamic Awarding Logic ---
                if (normalizedType === 'gift') {
                    if (itemId === 'gift_ring') {
                        userUpdate['inventory.rings'] = arrayUnion(itemId);
                    } else {
                        userUpdate[`inventory.giftCounts.${itemId}`] = firebase.firestore.FieldValue.increment(r.qty || 1);
                        userUpdate['inventory.gifts'] = arrayUnion(itemId);
                        userUpdate[expKey] = expiresAt;
                    }
                } else {
                    // Catch-all for frames, titles, badges, rings, and FUTURE types (vehicles, pets, etc.)
                    // Mapping to firestore inventory.[type]s pluralized
                    var inventoryKey = `inventory.${normalizedType}s`;
                    userUpdate[inventoryKey] = arrayUnion(itemId);
                    if (expiresAt) userUpdate[expKey] = expiresAt;
                }
            });
        ...
        myBundle.items.forEach(r => {
            var rIcon = r.icon || '🎁';
            var normalizedType = (r.type === 'titles') ? 'title' : (r.type === 'badges') ? 'badge' : r.type;
            if (normalizedType === 'frame' || normalizedType === 'title' || normalizedType === 'badge') {
                receiptParts.push(`${rIcon} ${r._computedDuration || '?'}d`);
            } else {
                receiptParts.push(`${r.qty || 1}× ${rIcon}`);
            }
        });
...
var handleGachaRoll = async ({ family, currentUID, currentUserData, mode = 'free', lang, onNotification }) => {
    ...
    try {
        ...
        // 🛡️ SECURITY: Use centralized service for Gacha currency delta (spends + wins)
        if (currencyDelta !== 0) {
            await window.SecurityService.applyCurrencyTransaction(currentUID, currencyDelta, `Family Gacha: ${picked.type === 'currency' ? 'Win' : 'Pull'}`, {
                pickedItem: picked,
                mode: mode
            });
        }
...
var createFamily = async ({ tribeName, tribeTag, tribeDesc, tribeEmblem, currentUID, currentUserData, lang }) => {
    ...
    if (window.SecurityService) {
        ...
        await usersCollection.doc(currentUID).update({ currency: firebase.firestore.FieldValue.increment(-FAMILY_CREATE_COST) });
    }
...
var handleDonate = async ({ family, amount, currentUID, currentUserData, lang }) => {
    ...
    /* 🛡️ SECURITY: Donation transaction */
    if (window.SecurityService) {
        await window.SecurityService.applyCurrencyTransaction(currentUID, -amount, 'Family Donation');
    } else {
        await usersCollection.doc(currentUID).update({ currency: firebase.firestore.FieldValue.increment(-amount) });
    }
...
var claimDailyMilestoneChest = async ({ family, currentUID, msIdx, ms, lang, onNotification }) => {
    ...
    try {
        ...
        for (const r of rewards) {
            if (r.type === 'currency' && r.amount > 0) {
                // 🛡️ SECURITY: Daily Milestone currency reward
                if (window.SecurityService) {
                    await window.SecurityService.applyCurrencyTransaction(currentUID, r.amount, 'Daily Milestone Reward');
                } else {
                    await usersCollection.doc(currentUID).update({ currency: firebase.firestore.FieldValue.increment(r.amount) });
                }
