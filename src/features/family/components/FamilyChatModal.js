...
var FamilyChatModal = (props) => {
  ...
  React.useEffect(() => {
    ...
    messages.forEach(msg => {
      var isRichGift = msg.type === 'system' && msg.text && msg.text.startsWith('__RICH_GIFT__|');
      if (isRichGift) {
        try {
      ...
      React.createElement('div', {
        ...
        React.createElement('div', {
          ...
          messages.map(function (msg) {
            ...
            if (msg.type === 'chest_assigned' || msg.type === 'chest_opened') {
              ...
              return React.createElement('div', { key: msg.id, style: { textAlign: 'center', padding: '10px 14px', margin: '4px 0' } },
                React.createElement('div', { style: { display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: '6px', background: `${chestColor}14`, border: `1px solid ${chestColor}44`, borderRadius: '14px', padding: '12px 20px', maxWidth: '320px', width: '100%' } },
                  ...
                  isAssigned && myClaimCount < maxClaims && React.createElement('div', {
                    style: { display: 'flex', alignItems: 'center', gap: '6px', background: chestColor + '22', border: '1px solid ' + chestColor + '55', borderRadius: '10px', padding: '5px 12px', cursor: 'pointer' },
                    onClick: function () {
                  ...
                  React.createElement('div', {
                    style: { fontSize: '10px', color: '#6b7280', cursor: 'pointer', textDecoration: 'underline dotted', marginTop: '2px' },
            ...
            // 🎁 Gift Card message (Detection with JSON decoding)
            var isRichGift = msg.type === 'system' && msg.text && msg.text.startsWith('__RICH_GIFT__|');
            var isLegacyGift = msg.type === 'gift' || (msg.type === 'system' && msg.giftId);

            if (isRichGift || isLegacyGift) {
              var gd = msg;
              if (isRichGift) {
                try {
                  gd = JSON.parse(msg.text.split('|')[1]);
                  gd.timestamp = msg.timestamp;
                  gd.id = msg.id;
                } catch (e) { gd = msg; }
              }

              var rarityCfg = (window.RARITY_CONFIG && window.RARITY_CONFIG[gd.rarity]) || { color: '#9ca3af', bg: 'rgba(156,163,175,0.07)', border: 'rgba(156,163,175,0.25)' };
              var cardBg = rarityCfg.bg || 'rgba(255,255,255,0.05)';
              var cardBorder = rarityCfg.border || 'rgba(255,255,255,0.12)';
              var accentColor = rarityCfg.color || '#fff';

              return React.createElement('div', { key: msg.id, style: { display: 'flex', justifyContent: 'center', padding: '10px 0', width: '100%', animation: 'fadeInScale 0.4s ease-out' } },
                React.createElement('div', {
                  style: {
                    width: '90%',
                    maxWidth: '280px',
                    background: 'linear-gradient(135deg, ' + cardBg + ', rgba(0,0,0,0.4))',
                    border: '1px solid ' + cardBorder,
                    borderRadius: '20px',
                    padding: '16px',
                    position: 'relative',
                    overflow: 'hidden',
                    boxShadow: rarityCfg.glow ? '0 0 25px ' + accentColor + '33' : 'none',
                    textAlign: 'center'
                  }
                },
                  // Rarity Glow Effect
                  rarityCfg.glow && React.createElement('div', { style: { position: 'absolute', top: '-20px', right: '-20px', width: '60px', height: '60px', background: accentColor, filter: 'blur(30px)', opacity: 0.3, borderRadius: '50%' } }),

                  // Header: Sender to Receiver
                  React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '15px' } },
                    gd.senderPhoto && React.createElement('img', { src: gd.senderPhoto, style: { width: '22px', height: '22px', borderRadius: '50%', border: '1.5px solid rgba(255,255,255,0.2)', objectFit: 'cover' } }),
                    React.createElement('span', { style: { fontSize: '10px', color: '#9ca3af', fontWeight: 600, maxWidth: '70px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, gd.senderName),
                    React.createElement('span', { style: { fontSize: '12px', filter: 'drop-shadow(0 0 5px rgba(255,255,255,0.3))' } }, '🎁'),
                    React.createElement('span', { style: { fontSize: '10px', color: '#9ca3af', fontWeight: 600, maxWidth: '70px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, gd.receiverName)
                  ),

                  // Gift Image/Emoji
                  React.createElement('div', { style: { position: 'relative', width: '80px', height: '80px', margin: '0 auto 12px', display: 'flex', alignItems: 'center', justifyContent: 'center' } },
                    gd.giftImageUrl ?
                      React.createElement('img', { src: gd.giftImageUrl, style: { width: '100%', height: '100%', objectFit: 'contain', filter: 'drop-shadow(0 5px 15px rgba(0,0,0,0.5))' } }) :
                      React.createElement('span', { style: { fontSize: '50px', filter: 'drop-shadow(0 5px 10px rgba(0,0,0,0.3))' } }, gd.giftEmoji || '🎁'),
                    gd.qty > 1 && React.createElement('div', { style: { position: 'absolute', bottom: '-2px', right: '-2px', background: 'linear-gradient(135deg, #ff416c, #ff4b2b)', color: '#fff', fontSize: '9px', fontWeight: 900, padding: '2px 6px', borderRadius: '8px', border: '1.5px solid rgba(0,0,0,0.5)', boxShadow: '0 2px 5px rgba(0,0,0,0.4)' } }, 'x' + gd.qty)
                  ),

                  // Gift Details
                  React.createElement('div', { style: { fontSize: '14px', fontWeight: 900, color: accentColor, marginBottom: '4px', textShadow: '0 2px 4px rgba(0,0,0,0.3)', letterSpacing: '0.5px' } }, lang === 'ar' ? gd.giftName_ar : gd.giftName_en),
                  React.createElement('div', { style: { fontSize: '10px', color: '#d1d5db', lineHeight: 1.4, marginBottom: '14px', padding: '0 8px', fontStyle: 'italic', opacity: 0.9 } }, lang === 'ar' ? gd.giftDesc_ar : gd.giftDesc_en),

                  // Stats Badge
                  React.createElement('div', { style: { display: 'flex', justifyContent: 'center', gap: '8px', background: 'rgba(0,0,0,0.25)', borderRadius: '12px', padding: '6px 12px', border: '1px solid rgba(255,255,255,0.05)' } },
                    React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '3px' } },
                      React.createElement('span', { style: { fontSize: '10px', fontWeight: 800, color: '#a78bfa' } }, '+' + (gd.charisma || gd.giftCharisma || 0)),
                      React.createElement('span', { style: { fontSize: '9px' } }, '⭐')
                    ),
                    (gd.totalBonus || gd.giftBonus || 0) > 0 && React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '3px', marginLeft: '4px', borderLeft: '1px solid rgba(255,255,255,0.1)', paddingLeft: '8px' } },
                      React.createElement('span', { style: { fontSize: '10px', fontWeight: 800, color: '#3b82f6' } }, '+' + (gd.totalBonus || gd.giftBonus || 0)),
                      React.createElement('span', { style: { fontSize: '9px' } }, '🧠')
                    )
                  ),

                  // Footer Time
                  React.createElement('div', { style: { fontSize: '8px', color: '#6b7280', marginTop: '12px', opacity: 0.7 } }, fmtTime(msg.timestamp))
                )
              );
            }
            // 🧧 Red Packet message
            ...
            if (isSystem && !msg.giftId && !isRichGift) return React.createElement('div', { key: msg.id, style: { textAlign: 'center', padding: '4px 12px' } },
        ...
        showChatGiftModal && onSendGift && SendGiftModal && React.createElement(SendGiftModal, {
          ...
          friendsData: [
            Object.assign({}, userData || currentUserData, { uid: currentUID }),
            ...familyMembers.filter(function (m) { return m.id !== currentUID; }).map(function (m) { return Object.assign({}, m, { uid: m.id }); })
          ]
        })
      )
    )
