...
var FamilyShop = ({
  ...
  return (/*#__PURE__*/
    React.createElement("div", { style: { flex: 1, overflowY: 'auto', padding: '14px', display: 'flex', flexDirection: 'column', gap: '12px' } }, /*#__PURE__*/


    React.createElement("div", { style: { ...localS.card, padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(135deg,rgba(15,15,35,0.9),rgba(26,26,50,0.9))', border: '1px solid rgba(167,139,250,0.25)' } }, /*#__PURE__*/
    React.createElement("div", null, /*#__PURE__*/
    React.createElement("div", { style: { fontSize: '10px', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' } },
    lang === 'ar' ? 'رصيد العائلة' : 'Family Balance'
    ), /*#__PURE__*/
    React.createElement("div", { style: { fontSize: '24px', fontWeight: 900, color: '#a78bfa', fontStyle: 'italic' } },
    coins, " ", coinsSymbol
    )
    ), /*#__PURE__*/
    ...
    React.createElement("div", { style: { display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '2px' } },
    TABS.map((tab) => {
      var active = activeType === tab.key;
      var count = tab.key === 'all' ? shopItems.length : shopItems.filter((i) => i.type === tab.key).length;
      return (/*#__PURE__*/
        React.createElement("button", { key: tab.key, onClick: () => setActiveType(tab.key), style: {
        ...
        tab.icon, " ", lang === 'ar' ? tab.label_ar : tab.label_en, /*#__PURE__*/
        React.createElement("span", { style: { background: active ? 'rgba(167,139,250,0.3)' : 'rgba(255,255,255,0.08)', borderRadius: '10px', padding: '0 5px', fontSize: '9px' } }, count)
        ));

    })
    ), /*#__PURE__*/

    React.createElement("div", { style: { ...localS.sectionTitle, marginTop: '4px' } }, "\uD83C\uDFC5 ",
    ...
    filtered.map((item) => {
      ...
      return (/*#__PURE__*/
        ...
        React.createElement("div", { style: { position: 'absolute', top: '6px', right: '6px', fontSize: '8px', fontWeight: 800, color: rColor, background: `${rColor}20`, padding: '1px 5px', borderRadius: '4px', textTransform: 'uppercase' } },
        item.rarity
        ),


        item.durationDays && /*#__PURE__*/
        React.createElement("div", { style: { position: 'absolute', top: '6px', left: '6px', fontSize: '8px', fontWeight: 800, color: '#f59e0b', background: 'rgba(245,158,11,0.15)', padding: '1px 5px', borderRadius: '4px' } },
        item.durationDays, lang === 'ar' ? 'ي' : 'd'
        ), /*#__PURE__*/


        React.createElement("div", { style: { fontSize: '32px', lineHeight: 1, marginTop: item.durationDays ? '8px' : '0', filter: owned ? 'none' : `drop-shadow(0 0 6px ${rColor}88)` } },
        item.emoji
        ), /*#__PURE__*/

        React.createElement("div", { style: { textAlign: 'center' } }, /*#__PURE__*/
        React.createElement("div", { style: { fontSize: '11px', fontWeight: 800, color: owned ? '#10b981' : canAfford ? 'white' : '#6b7280', marginBottom: '2px' } },
        lang === 'ar' ? item.name_ar : item.name_en
        ), /*#__PURE__*/
        React.createElement("div", { style: { fontSize: '9px', color: '#6b7280', lineHeight: 1.3, marginBottom: '4px' } },
        lang === 'ar' ? item.desc_ar : item.desc_en
        ),

        expiryLabel && /*#__PURE__*/
        React.createElement("div", { style: { fontSize: '9px', color: '#34d399', fontWeight: 700 } }, "\u23F0 ",
        expiryLabel
        ),


        purchasedButExpired && /*#__PURE__*/
        React.createElement("div", { style: { fontSize: '9px', color: '#f59e0b', fontWeight: 700 } },
        lang === 'ar' ? '🔄 تجديد' : '🔄 Renew'
        )
    ...
    React.createElement("div", { style: { fontSize: '12px' } }, lang === 'ar' ? 'لا عناصر في هذه الفئة' : 'No items in this category')
    ), /*#__PURE__*/
