    ...
    var ChatManager = {
        ...
        handleSendGiftToUser: async function(context, gift, targetUser, qty, fromInventory, familyScopeId) {
            ...
            try {
                ...
                if (window.SecurityService) {
                    await window.SecurityService.applyCurrencyTransaction(user.uid, -totalCost, `Gift Sent: ${gift.id} x${qty}`, { receiverId: isSelfSend ? user.uid : targetUser.uid });
                } else {
                    await usersCollection.doc(user.uid).update({ currency: firebase.firestore.FieldValue.increment(-totalCost) });
                }
        ...
        handlePurchase: async function(context, item, targetUser, qty) {
            ...
            if (item.type === 'red_packets') {
                ...
                // 🛡️ SECURITY: Red Packet Purchase
                if (window.SecurityService) {
                    await window.SecurityService.applyCurrencyTransaction(user.uid, -actualCost, `Purchase Red Packet Bundle: ${item.id}`);
                } else {
            ...
            if (item.type && typeof item.type === 'string' && item.type.startsWith('gifts')) {
                ...
                // 🛡️ SECURITY: Gift Purchase
                if (window.SecurityService) {
                    await window.SecurityService.applyCurrencyTransaction(user.uid, -actualCost, `Purchase Gift: ${item.id}`);
                } else {
                    await usersCollection.doc(user.uid).update({ currency: firebase.firestore.FieldValue.increment(-actualCost) });
                }
                try {
            ...
            if (item.cardType !== undefined) {
                // 🛡️ SECURITY: BFF Token Purchase
                if (window.SecurityService) {
                    await window.SecurityService.applyCurrencyTransaction(user.uid, -actualCost, `Purchase BFF Token: ${item.id}`);
                } else {
                    await usersCollection.doc(user.uid).update({ currency: firebase.firestore.FieldValue.increment(-actualCost) });
                }
                try {
            ...
            if (alreadyOwned) {
                if (item.durationDays && item.durationDays > 0) {
                    try {
                        ...
                        // 🛡️ SECURITY: Duration Item Renewal
                        if (window.SecurityService) {
                            await window.SecurityService.applyCurrencyTransaction(user.uid, -actualCost, `Renew Item: ${item.id} (+${item.durationDays}d)`);
                        } else {
                            await usersCollection.doc(user.uid).update({ currency: firebase.firestore.FieldValue.increment(-actualCost) });
                        }
                        await usersCollection.doc(user.uid).update({
            ...
            // 🛡️ SECURITY: Generic Item Purchase
            if (window.SecurityService) {
                await window.SecurityService.applyCurrencyTransaction(user.uid, -actualCost, `Purchase Item: ${item.id}`);
            } else {
                await usersCollection.doc(user.uid).update({ currency: firebase.firestore.FieldValue.increment(-actualCost) });
            }
            try {
        ...
        handleBuyVIP: async function(context) {
            ...
            try {
                ...
                // 🛡️ SECURITY: VIP Purchase/Renewal
                if (window.SecurityService) {
                    await window.SecurityService.applyCurrencyTransaction(user.uid, -VIP_SHOP_COST, isFirstPurchase ? 'VIP Purchase' : 'VIP Renewal');
                } else {
                    await usersCollection.doc(user.uid).update({ currency: firebase.firestore.FieldValue.increment(-VIP_SHOP_COST) });
                }
