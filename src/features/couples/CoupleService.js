  ...
  var sendProposal = async ({ fromUID, toUID, fromData, ringId, giftId, message, onNotification, lang }) => {
    ...
    try {
      ...
      if (window.SecurityService) {
        await window.SecurityService.applyCurrencyTransaction(fromUID, -totalCost, 'Wedding Ceremony');
      } else {
        await usersCollection.doc(fromUID).update({ currency: firebase.firestore.FieldValue.increment(-totalCost) });
      }
  ...
  var declineProposal = async ({ coupleDocId, fromUID, toUID, ringCost, giftCost, onNotification, lang }) => {
    try {
      ...
      if (window.SecurityService) {
        ...
        await usersCollection.doc(fromUID).update({ currency: firebase.firestore.FieldValue.increment(ringCost + (giftCost || 0)) });
      }
      await batch.commit();
