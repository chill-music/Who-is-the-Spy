                    ...
                    <div className="login-rewards-container">
                        ...
                        {canClaimToday && currentReward && (
                            <div style={{ marginTop: '12px', padding: '12px', background: 'linear-gradient(135deg, rgba(255, 215, 0, 0.1), rgba(255, 136, 0, 0.05))', border: '1px solid rgba(255, 215, 0, 0.3)', borderRadius: '12px', textAlign: 'center' }}>
                                <div style={{ fontSize: '10px', color: 'rgba(255, 255, 255, 0.5)', marginBottom: '4px' }}>{lang === 'ar' ? '��افأة ا����' : "Today's Reward"}</div>
                                <div style={{ fontSize: '14px', fontWeight: '700', color: 'white' }}>{renderRewardIcon(currentReward, 18)} {getRewardName(currentReward)}</div>
                                {currentReward.special && <div style={{ fontSize: '10px', color: '#ffd700', marginTop: '4px' }}>⭐ {lang === 'ar' ? '��افأة خاصة!' : 'Special Reward!'}</div>}
                            </div>
                        )}
                        ...
                        {!canClaimToday && countdown && (
                            <div style={{textAlign:'center', marginTop:'10px', padding:'8px 12px', background:'rgba(0,242,255,0.06)', border:'1px solid rgba(0,242,255,0.15)', borderRadius:'10px'}}>
                                <div style={{fontSize:'9px', color:'#64748b', marginBottom:'3px'}}>
                                    {lang === 'ar' ? '⏳ ا���ت ا��تب�� ����� ا�تا��' : '⏳ Next reward in'}
                                </div>
    ...
    const handlePasswordJoin = () => {
        if (selectedRoom && joinPassword) {
            if (joinPassword !== selectedRoom.password) {
                setPasswordError(lang === 'ar' ? '���ة ا�سر غ�ر صح�حة!' : 'Incorrect password!');
                return;
            }
            ...
            <div className="modal-content animate-pop" onClick={e => e.stopPropagation()} style={{ maxWidth: '400px' }}>
                        ...
                        <div className="browse-rooms-container">
                            {rooms.map(room => (
                                <div key={room.id} className="room-card">
                                    <div className="room-card-header"><span className="room-card-code">{room.id}</span><span className="room-card-mode">{room.mode === 'advanced' ? (lang === 'ar' ? '�ت�د�' : 'Advanced') : (lang === 'ar' ? 'عاد�' : 'Normal')}</span></div>
                ...
                {showPasswordInput && selectedRoom && (
                    <div className="p-3 bg-white/5 border-t border-white/10">
                        <div className="text-xs text-gray-400 mb-2">� {lang === 'ar' ? 'أدخ� ���ة ا�سر' : 'Enter password'} - Room: {selectedRoom.id}</div>
...
var upportTicketSection = ({ user, userData, lang, onNotification }) => {
    ...
    const handleSubmitTicket = async () => {
        ...
        try {
            ...
            setView('list');
            onNotification(lang==='ar'?'� ت� إرسا� تذ�رت�!':'� Ticket submitted!');
        } catch(e) {
    ...
    const handleUserReply = async () => {
        ...
        try {
            ...
            setUserReply('');
            onNotification(lang==='ar'?'� ت� إرسا� رد�':'� Reply sent');
        } catch(e) { onNotification('� Error'); }
        setSendingReply(false);
    };

    const statusCfg = {
        open:     { label_ar:'�فت�ح',    label_en:'Open',     color:'#ef4444' },
        answered: { label_ar:'ت� ا�رد',  label_en:'Answered', color:'#f59e0b' },
        closed:   { label_ar:'�غ��',     label_en:'Closed',   color:'#6b7280' },
    };
    const catIcon = { bug:'�', account:'�', payment:'�', other:'�' };
    const catLabels_ar = { bug:'خطأ ت���', account:'حساب', payment:'دفع', other:'أخر�' };
        ...
        return (
            <div className="settings-section">
                <div className="settings-section-title">
                    <span>�</span>
                    <span>{lang==='ar'?'تفاص�� ا�تذ�رة':'Ticket Details'}</span>
                </div>
                ...
                <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'10px', padding:'12px' }}>
                    ...
                    <div style={{ background:'rgba(112,0,255,0.05)', border:'1px solid rgba(112,0,255,0.15)', borderRadius:'7px', padding:'10px', marginBottom:'10px', fontSize:'12px', color:'#d1d5db' }}>
                        <div style={{ fontSize:'10px', color:'#7c3aed', fontWeight:700, marginBottom:'4px' }}>� {lang==='ar'?'رسا�ت�':'Your message'}</div>
                        {selected.message}
                    </div>
                    {/* Thread */}
                    ...
                    {selected.status !== 'closed' && (
                        <div style={{ marginTop:'10px' }}>
                            <textarea
                                ...
                                placeholder={lang==='ar'?'أضف تع���ا� أ� �ع���ات إضاف�ة...':'Add a comment or more info...'}
                                value={userReply}
                            ...
                            <button onClick={handleUserReply} disabled={sendingReply || !userReply.trim()}
                                ...
                                {sendingReply ? '⏳' : `� ${lang==='ar'?'إرسا�':'Send'}`}
                            </button>
                        </div>
                    )}
                    {selected.status === 'closed' && (
                        <div style={{ textAlign:'center', fontSize:'11px', color:'#6b7280', padding:'8px', borderTop:'1px solid rgba(255,255,255,0.06)', marginTop:'8px' }}>
                            � {lang==='ar'?'�ذ� ا�تذ�رة �غ��ة':'This ticket is closed'}
                        </div>
                    )}
                </div>
            </div>
    ...
    if (view === 'new') {
        return (
            <div className="settings-section">
                <div className="settings-section-title">
                    <span>�</span>
                    <span>{lang==='ar'?'تذ�رة دع� جد�دة':'New Support Ticket'}</span>
                </div>
                <button onClick={() => setView('list')}
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                    {/* Category */}
                    <div>
                        <div style={{ fontSize:'10px', color:'#9ca3af', marginBottom:'5px' }}>{lang==='ar'?'��ع ا��ش��ة:':'Category:'}</div>
                    ...
                    {/* Subject */}
                    <div>
                        <div style={{ fontSize:'10px', color:'#9ca3af', marginBottom:'5px' }}>{lang==='ar'?'ع��ا� ا��ش��ة:':'Subject:'}</div>
                        <input
                            style={{ width:'100%', padding:'8px 10px', borderRadius:'7px', fontSize:'12px',
                                background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', color:'white', outline:'none' }}
                            placeholder={lang==='ar'?'�صف �ختصر ���ش��ة...':'Brief description of your issue...'}
                            value={subject}
                            onChange={e => setSubject(e.target.value)} />
                    </div>
                    {/* Message */}
                    <div>
                        <div style={{ fontSize:'10px', color:'#9ca3af', marginBottom:'5px' }}>{lang==='ar'?'تفاص�� ا��ش��ة:':'Details:'}</div>
                        <textarea
                            style={{ width:'100%', padding:'8px 10px', borderRadius:'7px', fontSize:'12px', minHeight:'80px', resize:'vertical',
                                background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', color:'white', outline:'none' }}
                            placeholder={lang==='ar'?'اشرح �ش��ت� با�تفص��...':'Describe your issue in detail...'}
                            value={message}
                    ...
                    <button onClick={handleSubmitTicket} disabled={submitting || !subject.trim() || !message.trim()}
                        ...
                        {submitting ? '⏳' : `� ${lang==='ar'?'إرسا� ا�تذ�رة':'Submit Ticket'}`}
                    </button>
                </div>
            </div>
        );
        ...
        <div className="settings-section">
            <div className="settings-section-title" style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                    <span>�</span>
                    <span>{lang==='ar'?'ا�دع� ا�ف��':'Support'}</span>
                    {openCount > 0 && (
                ...
                <button onClick={() => setView('new')}
                    ...
                    + {lang==='ar'?'تذ�رة جد�دة':'New Ticket'}
                </button>
            </div>
            ...
            {loading ? (
                ...
                <div style={{ textAlign:'center', padding:'20px', color:'#6b7280' }}>
                    ...
                    <div style={{ fontSize:'12px', marginBottom:'10px' }}>{lang==='ar'?'��س �د�� تذا�ر دع� بعد':'No support tickets yet'}</div>
                    <button onClick={() => setView('new')}
                        style={{ fontSize:'11px', padding:'6px 16px', borderRadius:'7px', cursor:'pointer', fontWeight:700,
                            background:'rgba(0,242,255,0.1)', border:'1px solid rgba(0,242,255,0.25)', color:'#00f2ff' }}>
                        + {lang==='ar'?'فتح تذ�رة':'Open Ticket'}
                    </button>
                </div>
            ) : (
                        ...
                        return (
                            <div key={ticket.id} onClick={() => { setSelected(ticket); setView('detail'); }}
                                ...
                                <div style={{ fontSize:'10px', color:'#6b7280' }}>
                                    {(ticket.responses||[]).length} {lang==='ar'?'رد�د':'replies'}
...
var SettingsModal = ({ show, onClose, lang, onSetLang, userData, user, onNotification, isGuest: isGuestPropForSettings, onLoginGoogle, onOpenAdminPanel }) => {
    ...
    const toggleSound = () => {
        ...
        onNotification(newMuted ? (lang === 'ar' ? 'ت� �ت� ا�ص�ت' : 'Sound muted') : (lang === 'ar' ? 'ت� تشغ�� ا�ص�ت' : 'Sound enabled'));
    };
    ...
    const handleBlockUser = async () => {
        ...
        try {
            ...
            if (targetUid === user.uid) {
                onNotification(lang === 'ar' ? '�ا ����� حظر �فس�' : 'Cannot block yourself');
                setLoading(false);
                return;
            }
            if (blockedUsers.includes(targetUid)) {
                onNotification(lang === 'ar' ? 'ا��ستخد� �حظ�ر با�فع�' : 'User already blocked');
                setLoading(false);
                return;
            }
            // Add to blocked list
                ...
                <div className="modal-body">
                    ...
                    {isGuestPropForSettings && (
                        <div className="settings-section">
                            <div className="settings-section-title">
                                <span>�</span>
                                <span>{lang === 'ar' ? '�ع���ات ا�حساب' : 'Account Info'}</span>
                            </div>
                            <div style={{padding:'16px', borderRadius:'12px', background:'linear-gradient(135deg,rgba(0,10,30,0.6),rgba(20,0,50,0.4))', border:'1px solid rgba(0,242,255,0.15)', textAlign:'center'}}>
                                ...
                                <div style={{fontSize:'11px', color:'#6b7280', marginBottom:'14px'}}>
                                    {lang === 'ar' ? 'سج�� دخ��� ���ص�� �ج��ع ا����زات' : 'Login to access all features'}
                                </div>
                                <button onClick={onLoginGoogle} style={{
                                    ...
                                    {lang === 'ar' ? 'تسج�� ا�دخ�� بج�ج�' : 'Login with Google'}
                                </button>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="settings-section">
                                    ...
                                    reader.onload = async(ev) => {
                                        ...
                                        img.onload = async() => {
                                            ...
                                            try {
                                                ...
                                                onNotification(lang==='ar'?'ت� تحد�ث ا�ص�رة!':'Photo updated!');
                                            } catch(err){ }
                                        };
                            ...
                            <div className="settings-account-card">
                                <div className="settings-account-row">
                                    <span className="settings-account-label">� {lang === 'ar' ? 'ا�بر�د' : 'Email'}</span>
                                    <div style={{display:'flex',alignItems:'center',gap:'6px'}}>
                                ...
                                <div className="settings-account-row">
                                    <span className="settings-account-label">� {lang === 'ar' ? 'عض� ��ذ' : 'Member Since'}</span>
                                    <span className="settings-account-value">{userData?.createdAt?.toDate?.() ? userData.createdAt.toDate().toLocaleDateString() : 'N/A'}</span>
                                </div>
                                <div className="settings-account-row">
                                    <span className="settings-account-label">� {lang === 'ar' ? '��ع ا�حساب' : 'Account Type'}</span>
                                    <span className="settings-account-value" style={{color:'#10b981'}}>Google �</span>
                                </div>
                                <div className="settings-account-row">
                                    <span className="settings-account-label">�️ {lang === 'ar' ? 'ا�اس�' : 'Name'}</span>
                                    {editingName ? (
                                        <div style={{display:'flex',gap:'4px'}}>
                                            ...
                                            <button className="btn-neon" style={{padding:'2px 8px',fontSize:'10px',borderRadius:'6px'}} onClick={async() => {
                                                if(newName.trim() && user) {
                                                    ...
                                                    if(diffDays < 30) { onNotification(lang==='ar'?'���� ا�تغ��ر �رة ش�ر��ا':'Can change once per month'); setEditingName(false); return; }
                                                    await usersCollection.doc(user.uid).update({displayName:newName.trim(), lastChangedName:TS()});
                                                    onNotification(lang==='ar'?'ت� تغ��ر ا�اس�!':'Name changed!');
                            ...
                            <div style={{marginTop:'8px'}}>
                                ...
                                <CountryPicker
                                    ...
                                    onSelect={async (c) => {
                                        ...
                                        onNotification(lang === 'ar' ? 'ت� حفظ ا�ب�د �' : 'Country saved �');
                                    }}
                                    lang={lang}
                                />
                            </div>
                            ...
                            <div style={{marginTop:'12px',paddingTop:'12px',borderTop:'1px solid rgba(255,255,255,0.06)'}}>
                                <div style={{fontSize:'11px',color:'#9ca3af',marginBottom:'8px'}}>� {lang==='ar'?'ا��غة':'Language'}</div>
                        ...
                        <div style={{marginBottom:'10px'}}>
                            <button onClick={() => { onClose(); if(onOpenAdminPanel) onOpenAdminPanel(); }}
                                ...
                                <div style={{flex:1,textAlign:lang==='ar'?'right':'left'}}>
                                    <div style={{fontSize:'13px',fontWeight:800,color:'#ffd700'}}>{lang==='ar'?'��حة ا�إدارة':'Admin Panel'}</div>
                                    <div style={{fontSize:'10px',color:'#9ca3af',marginTop:'1px'}}>{lang==='ar'?`دخ�� �� ${ROLE_CONFIG[getUserRole(userData,user?.uid)]?.label_ar}`:`Access as ${ROLE_CONFIG[getUserRole(userData,user?.uid)]?.label_en}`}</div>
                                </div>
...
var RedPacketCard = ({ rpId, rpAmount, maxClaims, senderName, currentUID, user, currentUser, lang, onClaim }) => {
        ...
        <div style={{background:'linear-gradient(135deg,rgba(239,68,68,0.15),rgba(185,28,28,0.1))',border:'1px solid rgba(239,68,68,0.4)',borderRadius:'16px',padding:'12px 14px',minWidth:'200px',maxWidth:'min(260px,calc(100vw-90px))'}}>
            <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px'}}>
                ...
                <div>
                    ...
                    <div style={{fontSize:'10px',color:'#fca5a5'}}>{rpAmount?.toLocaleString()} � · {lang==='ar'?'��':'from'} {senderName}</div>
                </div>
            ...
            <div style={{marginBottom:'8px'}}>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:'9px',color:'#9ca3af',marginBottom:'3px'}}>
                    <span>{lang==='ar'?'ت� ا�است�ا�':'Claimed'}: {claimedCount}/{rpData.maxClaims||maxClaims}</span>
                    <span>{lang==='ar'?'�تب��':'Remaining'}: {remaining?.toLocaleString?.()} �</span>
                </div>
            ...
            {!claimed && !exhausted && (
                <button onClick={()=>{onClaim(rpId);setShowDetails(false);}} style={{width:'100%',padding:'8px',borderRadius:'10px',background:'linear-gradient(135deg,rgba(239,68,68,0.3),rgba(185,28,28,0.2))',border:'1px solid rgba(239,68,68,0.5)',color:'#ffd700',fontSize:'12px',fontWeight:800,cursor:'pointer'}}>
                    �� {lang==='ar'?'است�� ا�آ�':'Claim Now'}
                </button>
            )}
            {claimed && <div style={{textAlign:'center',fontSize:'11px',color:'#4ade80',padding:'6px'}}>� {lang==='ar'?'است��ت�':'Claimed'}</div>}
            {isReclaimed && !claimed && <div style={{textAlign:'center',fontSize:'11px',color:'#f87171',padding:'6px'}}>� {lang==='ar'?'ت� استرداد ا��غ�ف':'Packet reclaimed'}</div>}
            {exhausted && !claimed && !isReclaimed && <div style={{textAlign:'center',fontSize:'11px',color:'#6b7280',padding:'6px'}}>� {lang==='ar'?'�فد ا��غ�ف':'Exhausted'}</div>}
        </div>
    );
    ...
    return (
        <button onClick={fetchDetails} style={{
            ...
            <div>
                <div style={{fontSize:'12px',fontWeight:800,color: claimed ? '#6b7280' : '#ffd700'}}>{lang==='ar'?'�غ�ف أح�ر':'Red Packet'}</div>
                <div style={{fontSize:'10px',color: claimed ? '#4b5563' : '#fca5a5',marginTop:'2px'}}>{rpAmount?.toLocaleString()} � · {maxClaims} {lang==='ar'?'است�ا�':'claims'}</div>
                <div style={{fontSize:'9px',color: claimed ? '#374151' : 'rgba(252,165,165,0.6)',marginTop:'2px'}}>
                    {claimed ? (lang==='ar'?'� ت� ا�است�ا�':'� Claimed') : (lang==='ar'?'اضغط ��تفاص��':'Tap for details')}
                </div>
            </div>
        </button>
    );
            ...
            <div style={{height:'clamp(140px,25vw,200px)',overflowY:'auto',padding:'10px 10px 4px',display:'flex',flexDirection:'column',gap:'4px'}}>
                {visibleMsgs.length === 0 && (
                    <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',color:'#4b5563',fontSize:'11px',flexDirection:'column',gap:'4px'}}>
                        ...
                        <span>{lang==='ar'?'�ا رسائ� بعد � �� ا�أ��!':'No messages yet � be the first!'}</span>
                    </div>
                )}
                {visibleMsgs.map((msg, i) => {
                    ...
                    if (msg.type === 'red_packet') return (
                        <div key={msg.id||i} style={{alignSelf:'center',fontSize:'10px',color:'#ffd700',padding:'3px 10px',background:'rgba(239,68,68,0.08)',borderRadius:'12px',border:'1px solid rgba(239,68,68,0.2)'}}>
                            � {msg.senderName} {lang==='ar'?'أرس� �غ�ف':'sent a packet'} {msg.rpAmount?.toLocaleString()} �
                        </div>
                    );
                    return (
            ...
            {isLoggedIn ? (
                <div style={{display:'flex',gap:'4px',padding:'7px 8px',borderTop:'1px solid rgba(255,255,255,0.06)',background:'rgba(0,0,0,0.3)',boxSizing:'border-box',width:'100%'}}>
                    <input ref={inputRef} value={msgText} onChange={e=>setMsgText(e.target.value)}
                        ...
                        placeholder={lang==='ar'?'ا�تب...':'Write...'}/>
                    ...
                    <button onClick={onOpenFull}
                        style={{width:'34px',height:'34px',borderRadius:'10px',border:'1px solid rgba(74,222,128,0.3)',background:'rgba(74,222,128,0.08)',color:'#4ade80',fontSize:'13px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}} title={lang==='ar'?'فتح �ا��':'Open full'}>�</button>
                </div>
            ) : (
                <div style={{padding:'8px 12px',borderTop:'1px solid rgba(255,255,255,0.06)',textAlign:'center',color:'#6b7280',fontSize:'11px'}}>
                    � {lang==='ar'?'سج�� دخ�� ���تابة':'Login to chat'}
                    <button onClick={onOpenFull} style={{marginLeft:'8px',padding:'3px 10px',borderRadius:'8px',background:'rgba(74,222,128,0.1)',border:'1px solid rgba(74,222,128,0.3)',color:'#4ade80',fontSize:'10px',fontWeight:700,cursor:'pointer'}}>{lang==='ar'?'�راءة':'Read'}</button>
                </div>
            )}
        </div>
    );
                ...
                <div onClick={e=>e.stopPropagation()} style={{
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 18px',borderBottom:'1px solid rgba(255,215,0,0.15)',background:'linear-gradient(135deg,rgba(255,215,0,0.08),rgba(255,140,0,0.04))',flexShrink:0}}>
                        <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                            ...
                            <div>
                                <div style={{fontSize:'16px',fontWeight:900,color:'#ffd700',letterSpacing:'0.02em'}}>{lang==='ar'?'�ر�ز VIP':'VIP Center'}</div>
                                <div style={{fontSize:'10px',color:'#9ca3af',marginTop:'1px'}}>{lang==='ar'?'ا�ت�ازات� ا�حصر�ة':'Your exclusive privileges'}</div>
                            </div>
                        </div>
                    ...
                    <div style={{flex:1,overflowY:'auto',padding:'12px 0'}}>
                        ...
                        <div style={{padding:'14px 16px 8px'}}>
                            <button onClick={()=>{onClose();if(onOpenShop)onOpenShop();}}
                                ...
                                � {lang==='ar'?'اشتر� VIP �� ا��تجر':'Buy VIP from Shop'}
                            </button>
                        </div>
                    </div>
                </div>
...
var HelpCenterModal = ({ show, onClose, user, userData, lang, onNotification, isLoggedIn }) => {
    ...
    const handleSendFeedback = async () => {
        ...
        try {
            ...
            setFeedbackText('');
            setFeedbackRating(5);
            onNotification(lang==='ar' ? '� ش�را� ع�� رأ��!' : '� Thanks for your feedback!');
    ...
    const tabs = [
        { id: 'faq',      icon: '�', label_ar: 'ا�أسئ�ة ا�شائعة', label_en: 'FAQ' },
        { id: 'tickets',  icon: '�', label_ar: 'ا�تذا�ر',          label_en: 'Tickets' },
        { id: 'feedback', icon: '�', label_ar: 'ف�دبا�',           label_en: 'Feedback' },
    ];
                ...
                <div onClick={e=>e.stopPropagation()} style={{
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 18px',borderBottom:'1px solid rgba(255,255,255,0.07)',flexShrink:0}}>
                        <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                            ...
                            <div>
                                <div style={{fontSize:'16px',fontWeight:900,color:'#e2e8f0'}}>{lang==='ar'?'�ر�ز ا��ساعدة':'Help Center'}</div>
                                <div style={{fontSize:'10px',color:'#6b7280',marginTop:'1px'}}>{lang==='ar'?'أسئ�ة� تذا�ر� آراء':'FAQ, tickets & feedback'}</div>
                            </div>
                        </div>
                        <button onClick={onClose} style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'10px',padding:'6px 10px',color:'#9ca3af',fontSize:'16px',cursor:'pointer'}}>�</button>
                    </div>
                            ...
                            <div style={{padding:'14px 16px'}}>
                                {loadingFaqs ? (
                                    ...
                                    <div style={{textAlign:'center',padding:'40px',color:'#4b5563'}}>
                                        ...
                                        <div style={{fontSize:'13px',fontWeight:600,color:'#6b7280'}}>{lang==='ar'?'�ا ت�جد أسئ�ة بعد':'No FAQs yet'}</div>
                                    </div>
                                ) : faqs.map(faq => (
                            ...
                            <div>
                                {!isLoggedIn ? (
                                    <div style={{textAlign:'center',padding:'40px 24px',color:'#6b7280'}}>
                                        <div style={{fontSize:'36px',marginBottom:'12px'}}>�</div>
                                        <div style={{fontSize:'13px'}}>{lang==='ar'?'�جب تسج�� ا�دخ�� �إرسا� تذ�رة':'Login required to submit tickets'}</div>
                                    </div>
                                ) : (
                                    <div style={{padding:'0'}}>
                        ...
                        {activeTab === 'feedback' && (
                            <div style={{padding:'16px'}}>
                                {!isLoggedIn ? (
                                    <div style={{textAlign:'center',padding:'40px 24px',color:'#6b7280'}}>
                                        <div style={{fontSize:'36px',marginBottom:'12px'}}>�</div>
                                        <div style={{fontSize:'13px'}}>{lang==='ar'?'�جب تسج�� ا�دخ��':'Login required'}</div>
                                    </div>
                                ) : (
                                    <>
                                        <div style={{marginBottom:'16px',textAlign:'center'}}>
                                            <div style={{fontSize:'14px',fontWeight:700,color:'#e2e8f0',marginBottom:'6px'}}>� {lang==='ar'?'شار��ا رأ��':'Share your feedback'}</div>
                                            <div style={{fontSize:'11px',color:'#6b7280'}}>{lang==='ar'?'رأ�� �ساعد�ا ع�� ا�تحس��':'Your opinion helps us improve'}</div>
                                        </div>
                                        {/* Rating stars */}
                                        ...
                                        <textarea value={feedbackText} onChange={e=>setFeedbackText(e.target.value.slice(0,500))}
                                            ...
                                            placeholder={lang==='ar'?'ا�تب رأ�� ��ا...':'Write your feedback here...'} />
                                        ...
                                        <button onClick={handleSendFeedback} disabled={sendingFeedback||!feedbackText.trim()}
                                            ...
                                            {sendingFeedback?'⏳...':`� ${lang==='ar'?'إرسا� ا�ف�دبا�':'Send Feedback'}`}
                                        </button>
                                    </>
                                )}
                            </div>
...
var PublicChatModal = ({ show, onClose, currentUser, user, lang, onNotification, isLoggedIn, onOpenProfile, currentUID }) => {
    ...
    const handleImgUpload = async (e) => {
        ...
        } catch(e) { onNotification(lang==='ar'?'� فش� رفع ا�ص�رة':'� Image upload failed'); }
    ...
    const handleReport = async (msg) => {
        ...
        try {
            ...
            onNotification(lang==='ar'?'� ت� إرسا� ا�ب�اغ':'� Report sent');
        } catch(e) {}
        setMenuMsgId(null);
    };
    ...
    const sendPublicRP = async (rp) => {
        ...
        if ((currentUser?.currency||0) < rp.amount) { onNotification(lang==='ar'?'� رص�د غ�ر �افٍ':'� Insufficient balance'); return; }
        setSendingRP(true);
        try {
            ...
            onNotification(lang==='ar'?'� ت� إرسا� ا��غ�ف!':'� Packet sent!');
        } catch(e) { onNotification(lang==='ar'?'� خطأ':'� Error'); }
        setSendingRP(false);
    };
    ...
    const claimPublicRP = async (rpId) => {
        ...
        try {
            ...
            const rp = rpDoc.data();
            // � ا��رس� �ا �ستط�ع ا�است�ا� �� �غ�ف� ف� ا�شات ا�عا�
            if (rp.senderId === user.uid) { onNotification(lang==='ar'?'� �ا ����� است�ا� �غ�ف� ا�خاص':'� You cannot claim your own packet'); return; }
            if (rp.claimedBy?.includes(user.uid)) { onNotification(lang==='ar'?'� است��ت� �� �ب�':'� Already claimed'); return; }
            if ((rp.claimedBy?.length||0) >= rp.maxClaims) { onNotification(lang==='ar'?'� �فد ا��غ�ف':'� Exhausted'); return; }
            ...
            await publicChatCollection.add({
                type: 'system',
                text: lang==='ar' ? `� ${currentUser?.displayName||'�ستخد�'} است�� ${claim} � �� �غ�ف ${rp.senderName}` : `� ${currentUser?.displayName||'User'} claimed ${claim} � from ${rp.senderName}'s packet`,
                ...
                <div onClick={e=>e.stopPropagation()} style={{
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 16px',borderBottom:'1px solid rgba(255,255,255,0.07)',flexShrink:0,background:'rgba(0,0,0,0.3)'}}>
                        <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                            ...
                            <div>
                                <div style={{fontSize:'15px',fontWeight:800,color:'#e2e8f0'}}>{lang==='ar'?'ا�شات ا�عا�':'Public Chat'}</div>
                                <div style={{fontSize:'10px',color:'#6b7280',marginTop:'1px'}}>{lang==='ar'?'ا�ج��ع ��ا':'Everyone is here'}</div>
                            </div>
                        </div>
                        <button onClick={onClose} style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'10px',padding:'6px 10px',color:'#9ca3af',fontSize:'16px',cursor:'pointer'}}>�</button>
                    </div>
                    ...
                    <div style={{flex:1,overflowY:'auto',padding:'10px',display:'flex',flexDirection:'column',gap:'6px'}}>
                            ...
                            if (msg.type === 'red_packet') {
                                ...
                                return (
                                    <div key={msg.id||i} style={{display:'flex',flexDirection:isMe?'row-reverse':'row',gap:'8px',alignItems:'flex-end'}}>
                                        ...
                                        <div>
                                            <div style={{fontSize:'10px',fontWeight:700,marginBottom:'3px',paddingLeft:isMe?0:'4px',paddingRight:isMe?'4px':0,textAlign:isMe?'right':'left',cursor:'pointer',color:'#fca5a5'}} onClick={()=>openMiniProfilePub(msg.senderId,{name:msg.senderName,photo:msg.senderPhoto})}>{isMe?(lang==='ar'?'أ�ت':'You'):msg.senderName}</div>
                                ...
                                <div key={msg.id||i} style={{display:'flex',flexDirection:isMe?'row-reverse':'row',gap:'8px',alignItems:'flex-end'}}>
                                    ...
                                    <div style={{maxWidth:'min(72%, calc(100vw - 80px))',minWidth:0}}>
                                        ...
                                        <div style={{display:'flex',alignItems:'center',gap:'4px',marginBottom:'3px',paddingLeft:isMe?0:'2px',paddingRight:isMe?'2px':0,justifyContent:isMe?'flex-end':'flex-start',flexWrap:'wrap',cursor:'pointer'}} onClick={()=>openMiniProfilePub(msg.senderId,{name:msg.senderName,photo:msg.senderPhoto})}>
                                            <span style={{fontSize:'10px',color:nameColor,fontWeight:800}}>{isMe?(lang==='ar'?'أ�ت':'You'):msg.senderName}</span>
                                            {msg.senderFamilyName && typeof FamilySignBadge !== 'undefined' && (
                                        ...
                                        {editingMsgId === msg.id ? (
                                            ...
                                            <div style={{padding:'8px 12px',borderRadius:isMe?'14px 4px 14px 14px':'4px 14px 14px 14px',background:isMe?'linear-gradient(135deg,rgba(112,0,255,0.4),rgba(0,242,255,0.18))':'rgba(255,255,255,0.07)',border:isMe?'1px solid rgba(0,242,255,0.18)':'1px solid rgba(255,255,255,0.08)',fontSize:'12px',color:'#e2e8f0',lineHeight:1.5,wordBreak:'break-word'}}>
                                                {msg.text}
                                                {msg.edited && <span style={{fontSize:'9px',color:'#4b5563',marginLeft:'6px',fontStyle:'italic'}}>{lang==='ar'?'(�عد��)':'(edited)'}</span>}
                                            </div>
                                        )}
                                                ...
                                                <div style={{position:'absolute',background:'linear-gradient(160deg,#0e0e22,#13122a)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:'10px',padding:'5px',zIndex:999,boxShadow:'0 8px 24px rgba(0,0,0,0.9)',minWidth:'120px',right: isMe ? '0' : 'auto', left: isMe ? 'auto' : '0'}}>
                                                    {isMe && !editingMsgId && (
                                                        <button onClick={()=>{setEditingMsgId(msg.id);setEditText(msg.text||'');setMenuMsgId(null);}} style={{width:'100%',padding:'7px 10px',borderRadius:'7px',background:'none',border:'none',cursor:'pointer',fontSize:'11px',color:'#00f2ff',fontWeight:700,textAlign:'left',display:'flex',alignItems:'center',gap:'6px'}}
                                                            ...
                                                            onMouseLeave={e=>e.currentTarget.style.background='none'}>�️ {lang==='ar'?'تعد��':'Edit'}</button>
                                                    )}
                                                    {isMe && (
                                                    ...
                                                    <button onClick={()=>handleReport(msg)} style={{width:'100%',padding:'7px 10px',borderRadius:'7px',background:'none',border:'none',cursor:'pointer',fontSize:'11px',color:'#9ca3af',fontWeight:700,textAlign:'left',display:'flex',alignItems:'center',gap:'6px'}}
                                                        ...
                                                        onMouseLeave={e=>e.currentTarget.style.background='none'}>� {lang==='ar'?'إب�اغ':'Report'}</button>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                        ...
                        {messages.length===0 && <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:'8px',color:'#4b5563',paddingTop:'40px'}}><div style={{fontSize:'32px'}}>��</div><div style={{fontSize:'12px'}}>{lang==='ar'?'ابدأ ا��حادثة!':'Start chatting!'}</div></div>}
                        <div ref={messagesEndRef}/>
                    </div>
                        ...
                        <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.7)',zIndex:80,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-end'}}
                            ...
                            <div style={{width:'100%',background:'linear-gradient(160deg,#0e0e22,#13122a)',borderRadius:'20px 20px 0 0',border:'1px solid rgba(255,255,255,0.1)',overflow:'hidden',maxHeight:'60%',boxSizing:'border-box'}}
                                ...
                                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 16px',borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
                                    <div style={{fontSize:'13px',fontWeight:800,color:'#ef4444'}}>� {lang==='ar'?'أرس� �غ�ف ��ع���':'Send Public Red Packet'}</div>
                                ...
                                <div style={{padding:'12px',overflowY:'auto'}}>
                                    <div style={{fontSize:'11px',color:'#6b7280',marginBottom:'10px',textAlign:'center'}}>{lang==='ar'?'رص�د�':'Balance'}: <span style={{color:'#ffd700',fontWeight:700}}>{((currentUser?.currency)||0).toLocaleString()} �</span></div>
                    ...
                    {isLoggedIn ? (
                        <div style={{display:'flex',gap:'5px',padding:'8px',borderTop:'1px solid rgba(255,255,255,0.07)',flexShrink:0,background:'rgba(0,0,0,0.4)',position:'relative',boxSizing:'border-box',width:'100%'}}>
                            ...
                            {showEmojiPicker && (
                                <div style={{position:'absolute',bottom:'58px',left:0,right:0,background:'#0e1020',border:'1px solid rgba(255,255,255,0.09)',borderRadius:'14px 14px 0 0',padding:'10px',zIndex:10,boxShadow:'0 -14px 44px rgba(0,0,0,0.8)',maxHeight:'200px',overflowY:'auto'}}>
                                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:'6px'}}>
                                        <span style={{fontSize:'11px',fontWeight:700,color:'#00f2ff'}}>{lang==='ar'?'إ���ج�':'Emoji'}</span>
                            ...
                            <button onClick={()=>setShowRPModal(true)} style={{width:'36px',height:'36px',borderRadius:'10px',border:'1px solid rgba(239,68,68,0.3)',background:'rgba(239,68,68,0.1)',cursor:'pointer',fontSize:'17px',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}} title={lang==='ar'?'�غ�ف أح�ر':'Red Packet'}>�</button>
                            <input ref={inputRef} value={msgText} onChange={e=>setMsgText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&(e.preventDefault(),sendMsg())}
                                ...
                                placeholder={lang==='ar'?'ا�تب رسا�ة ��ع���...':'Write a public message...'}/>
