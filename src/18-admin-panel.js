...
const TicketsSection = ({ currentUser, currentUserData, lang, onNotification }) => {
        ...
        try {
            ...
            if (selectedTicket.userId) {
                await notificationsCollection.add({
                    toUserId: selectedTicket.userId,
                    type: 'ticket_reply',
                    message_en: `Your ticket has been answered by staff.`,
                    message_ar: `تم الرد على تذكرتك من قِبل الفريق.`,
                    message: `Ticket reply: ${replyText.slice(0,80)}`,
                    icon: '🎫',
                    read: false,
                    timestamp: firebase.firestore.FieldValue.serverTimestamp()
                });
            }
            await logStaffAction(currentUser.uid, currentUserData?.displayName, 'REPLY_TICKET', selectedTicket.userId, selectedTicket.userName, replyText.slice(0,100));
            setReplyText('');
            onNotification(`✅ ${lang==='ar'?'تم الرد':'Reply sent'}`);
            // Refresh selected ticket
            const doc = await ticketsCollection.doc(selectedTicket.id).get();
    ...
    const closeTicket = async (ticketId) => {
        try {
            await ticketsCollection.doc(ticketId).update({ status: 'closed', closedAt: firebase.firestore.FieldValue.serverTimestamp() });
            await logStaffAction(currentUser.uid, currentUserData?.displayName, 'CLOSE_TICKET', null, null, `Ticket ${ticketId}`);
            onNotification(`✅ ${lang==='ar'?'تم إغلاق التذكرة':'Ticket closed'}`);
            if (selectedTicket?.id === ticketId) setSelectedTicket(null);
        } catch(e) {}
    };

    const statusColor = { open:'#ef4444', answered:'#f59e0b', closed:'#6b7280' };

    if (selectedTicket) {
        return (
            <div>
                <button onClick={() => setSelectedTicket(null)} style={{ fontSize:'12px', color:'#00f2ff', background:'none', border:'none', cursor:'pointer', marginBottom:'12px' }}>
                    ← {lang==='ar'?'رجوع للتذاكر':'Back to tickets'}
                </button>
                <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'12px', padding:'14px', marginBottom:'12px' }}>
                    <div style={{ fontWeight:700, fontSize:'13px', marginBottom:'4px' }}>{selectedTicket.subject}</div>
                    <div style={{ fontSize:'11px', color:'#9ca3af', marginBottom:'10px' }}>
                        {lang==='ar'?'من:':'From:'} {selectedTicket.userName} • {selectedTicket.createdAt?.toDate?.()?.toLocaleDateString()}
                    </div>
                    <div style={{ fontSize:'12px', color:'#d1d5db', background:'rgba(255,255,255,0.03)', borderRadius:'8px', padding:'10px', marginBottom:'12px' }}>
                        {selectedTicket.message}
                    </div>
                    {/* Responses */}
                    {(selectedTicket.responses || []).map((r, i) => (
                        <div key={i} style={{ background:'rgba(0,242,255,0.05)', border:'1px solid rgba(0,242,255,0.15)', borderRadius:'8px', padding:'8px', marginBottom:'6px' }}>
                            <div style={{ fontSize:'10px', color:'#00f2ff', fontWeight:700, marginBottom:'3px' }}>
                                👮 {r.byName} — {new Date(r.timestamp).toLocaleString()}
                            </div>
                            <div style={{ fontSize:'12px', color:'#d1d5db' }}>{r.message}</div>
                        </div>
                    ))}
                    {selectedTicket.status !== 'closed' && (
                        <div>
                            <textarea className="input-dark" style={{ width:'100%', padding:'8px', borderRadius:'8px', fontSize:'12px', minHeight:'60px', resize:'vertical', marginBottom:'8px' }}
                                placeholder={lang==='ar'?'اكتب ردك...':'Type your reply...'}
                                value={replyText} onChange={e => setReplyText(e.target.value)} />
                            <div style={{ display:'flex', gap:'8px' }}>
                                <button onClick={handleReply} disabled={replying || !replyText.trim()}
                                    className="btn-neon" style={{ flex:1, padding:'7px', borderRadius:'8px', fontSize:'12px', fontWeight:700 }}>
                                    {replying?'⏳':`📨 ${lang==='ar'?'إرسال':'Reply'}`}
                                </button>
                                <button onClick={() => closeTicket(selectedTicket.id)}
                                    style={{ padding:'7px 12px', borderRadius:'8px', fontSize:'11px', cursor:'pointer', background:'rgba(107,114,128,0.15)', border:'1px solid rgba(107,114,128,0.3)', color:'#9ca3af' }}>
                                    🔒 {lang==='ar'?'إغلاق':'Close'}
                                </button>
                            </div>
                        </div>
    ...
    return (
        <div>
            <div style={{ fontSize:'13px', fontWeight:700, color:'#6366f1', marginBottom:'16px' }}>
                🎫 {lang==='ar'?'تذاكر الدعم':'Support Tickets'}
            </div>
            {loading ? <div style={{color:'#6b7280',fontSize:'12px',textAlign:'center',padding:'20px'}}>⏳</div> : (
                <div style={{ display:'flex', flexDirection:'column', gap:'8px', maxHeight:'55vh', overflowY:'auto' }}>
                    {tickets.length === 0 && <div style={{color:'#6b7280',fontSize:'12px',textAlign:'center',padding:'20px'}}>{lang==='ar'?'لا توجد تذاكر':'No tickets yet'}</div>}
                    {tickets.map(ticket => (
                        <div key={ticket.id} onClick={() => setSelectedTicket(ticket)}
                            style={{ background:'rgba(255,255,255,0.03)', border:`1px solid rgba(${ticket.status==='open'?'239,68,68':ticket.status==='answered'?'245,158,11':'107,114,128'},0.2)`,
                                borderRadius:'10px', padding:'12px', cursor:'pointer', transition:'all 0.15s' }}
                            onMouseEnter={e => e.currentTarget.style.background='rgba(255,255,255,0.06)'}
                            onMouseLeave={e => e.currentTarget.style.background='rgba(255,255,255,0.03)'}>
                            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'4px' }}>
                                <span style={{ fontSize:'12px', fontWeight:700 }}>{ticket.subject || 'No subject'}</span>
                            ...
                            <div style={{ fontSize:'11px', color:'#9ca3af' }}>
                                {ticket.userName} • {ticket.createdAt?.toDate?.()?.toLocaleDateString()}
                                {(ticket.responses||[]).length > 0 && ` • ${ticket.responses.length} ${lang==='ar'?'ردود':'replies'}`}
                            </div>
                        </div>
                    ))}
                </div>
