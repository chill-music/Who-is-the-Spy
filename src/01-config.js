...
var VIP_ID_ICONS = {
    6: 'icos/idvip.png', // ← VIP 6:  ضع رابط الأيقونة الذهبية هنا
    7: 'icos/idvip6.png', // ← VIP 7:  ضع رابط الأيقونة المميزة هنا
    8: 'icos/idvip7.png', // ← VIP 8:  ضع رابط الأيقونة المميزة هنا
    9: 'icos/idvip8.png', // ← VIP 9:  ضع رابط الأيقونة الأنيمشن هنا
    10: 'icos/idvip10.png', // ← VIP 10: ضع رابط الأيقونة الأسطورية هنا
};
// ════════════════════════════════════════════════════════
...
window.goldLogCollection = goldLogCollection;
...
var CHEST_CONFIG = {
    ...
    epic: {
        name_en: 'Super Chest', name_ar: 'صندوق ملحمي', icon: '💎', color: '#ffd700',
        rewards: [
            { type: 'currency', amount: 7800 },
            { type: 'coins', amount: 7800 },
            { type: 'titles', titleId: 'title_spy', duration: 1, qty: 1 },
            { type: 'titles', titleId: 'title_spy', duration: 3, qty: 1 },
            { type: 'titles', titleId: 'title_spy', duration: 7, qty: 1 },
            { type: 'frame', frameId: 'frame_img', duration: 1, qty: 1 },
            { type: 'frame', frameId: 'frame_img', duration: 3, qty: 1 },
            { type: 'frame', frameId: 'frame_img', duration: 7, qty: 1 },
            { type: 'frame', frameId: 'frame_rainbow', duration: 30, qty: 1 },
        ],
    },
    super: {
        name_en: 'Imperial Chest', name_ar: 'صندوق أسطوري', icon: '👑', color: '#f97316',
        rewards: [
            { type: 'currency', amount: 12000 },
            { type: 'coins', amount: 120000 },
...
var resolveRewardItem = function (reward) {
    ...
    var SI = window.SHOP_ITEMS; // ⚠️ Possible Race Condition during bootstrap
    ...
    if (resolved.type === 'currency') {
    ...
    } else if (resolved.type === 'gift' && resolved.giftId) {
        if (SI) {
        ...
        resolved.icon = resolved.icon || '🎁';
        resolved.label_en = resolved.label_en || 'Gift';
        resolved.label_ar = resolved.label_ar || 'هدية';

    } else if (resolved.type === 'frame' && resolved.frameId) {
        if (SI) {
        ...
        resolved.icon = resolved.icon || '🏅';
        resolved.label_en = resolved.label_en || 'Title';
        resolved.label_ar = resolved.label_ar || 'لقب';

    } else if ((resolved.type === 'badge' || resolved.type === 'badges') && resolved.badgeId) {
        if (SI) {
        ...
        resolved.icon = resolved.icon || '🏅';
        resolved.label_en = resolved.label_en || 'Badge';
        resolved.label_ar = resolved.label_ar || 'شارة';

    } else if (resolved.type === 'ring' && resolved.ringId) {
        if (SI) {
            var ring = (SI.rings || []).find(function (r) { return r.id === resolved.ringId; });
            if (ring) {
                resolved.icon = ring.icon || '💍';
        ...
        resolved.icon = resolved.icon || '💍';
        resolved.label_en = resolved.label_en || 'Ring';
        resolved.label_ar = resolved.label_ar || 'خاتم';
    } else if (resolved.type === 'effect' && resolved.effectId) {
        if (SI) {
        ...
        resolved.icon = resolved.icon || '✨';
        resolved.label_en = resolved.label_en || 'Effect';
        resolved.label_ar = resolved.label_ar || 'تأثير';
    }
...
// NOTE: Feature-specific configurations (GACHA, DAILY_TASKS, FAMILY_SHOP, VIP_ID_ICONS)
// are exported to window within their respective modules (e.g., 19-family.js, 17-vip.js)
// to avoid race conditions with 01-config.js load time.
...
// ╔══════════════════════════════════════════════════════════════╗
// ║  RACE-CONDITION GUARD UTILITIES                              ║
// ║  Safe polling for late-loading globals (SHOP_ITEMS, etc.)   ║
// ╚══════════════════════════════════════════════════════════════╝

/**
 * Polls window[name] until it is defined, then calls callback(value).
 * If maxWait elapses without resolution, logs a warning.
 *
 * @param {string}   name      Global variable name (e.g. 'SHOP_ITEMS')
 * @param {Function} callback  Called with window[name] once available
...
window.__waitForGlobal = function(name, callback, interval, maxWait) {
    ...
    var poll = setInterval(function() {
        ...
        if (window[name] !== undefined) {
            ...
            clearInterval(poll);
            console.warn('[__waitForGlobal] Timeout: window.' + name + ' never became available.');
        }
    }, interval);
};
 ...
 * callback receives an object { name: value, ... } for each name.
 *
 * @param {string[]} names     Array of global names to wait for
